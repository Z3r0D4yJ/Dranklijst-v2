...
async function invokeJoinNotification(body: Record<string, unknown>) {
...
export async function notifyLeidingOfJoinRequest(groupId: string, requesterName: string) {
  await invokeJoinNotification({ type: 'join_request', group_id: groupId, requester_name: requesterName })
}

export async function notifyLeidingOfInviteJoinRequest(code: string, requesterName: string) {
  await invokeJoinNotification({ type: 'join_request', invite_code: code.trim().toUpperCase(), requester_name: requesterName })
}

export async function notifyJoinRequestResolved(userId: string, groupName: string, approved: boolean) {
  await invokeJoinNotification({ type: 'join_resolved', user_id: userId, group_name: groupName, approved })
}
